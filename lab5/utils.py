...
# TODO: implement this function
def  plot_pred(x, cond, modules, args):
    modules['posterior'].hidden = modules['posterior'].init_hidden()        
    modules['frame_predictor'].hidden = modules['frame_predictor'].init_hidden()
    ...
    save_image(padded_image(x[0][0], True, 'Approx. \nposterior'), "./img/" + str(1) + "/"+str(0)+".png")
    x = x[0]
    obs = True 
    for idx in range(1, args.n_past + args.n_future):
        ...
        h = modules['encoder'](x)
        h_t = modules['encoder'](x[idx])[0].detach()
        ...
        if idx < args.n_past:
            modules['frame_predictor'](torch.cat([cond[idx-1], h[0].detach(), z_t], 1)) 
            x = x[idx]
            save_image(padded_image(x[idx][0], obs, 'Approx. \nposterior'), "./img/" + str(1) + "/"+str(idx)+".png")

        else:
            g_t = modules['frame_predictor'](torch.cat([cond[idx-1], h[0].detach(), z_t], 1)).detach()
    ...
    for num in range(3):
        ...
        modules['posterior'].hidden = modules['posterior'].init_hidden()
        obs = True
        generation[num].append(x[0])

        # save_image(padded_image(x[0][0], obs, f'Random\nSample {num + 1}'), "./img/" + str(num+3) + "/"+str(1)+".png")
        for l in range(0, args.n_past + 1):
            h_t[l] =  modules["encoder"](x[l])

        for i in range (1, args.n_past + args.n_future):
            if i < args.n_past:
                obs = False
                generation[num].append(x[i])
                # save_image(padded_image(x[i][0], obs, f'Random\nSample {num + 1}'), "./img/"+str(num +3)+"/"+str(i+1)+".png")
                z_t, mu, logvar = modules['posterior'](h_t[i][0].detach())
                modules['frame_predictor'](torch.cat([cond[i-1], h_t[i-1][0].detach(), z_t], 1)) 
            else:    
                z_t = torch.randn(args.batch_size, args.z_dim).cuda()
                g_t = modules['frame_predictor'](torch.cat([cond[i-1] ,h_t[1-1][0].detach(), z_t], 1)).detach()
